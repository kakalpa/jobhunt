...
## Core Frameworks: Mandatory Google X-Y-Z Standard

Every single resume bullet point MUST be constructed using the **Google X-Y-Z Formula** established by Laszlo Bock (former Google SVP of People Operations):

> **"Accomplished [X] as measured by [Y], by doing [Z]"**

### Anatomical Breakdown of Google X-Y-Z:
1. **Accomplished [X] (The Outcome / Impact):** Start immediately with an active power verb stating the business, security, or technical result achieved. NEVER start with passive job duties like "Responsible for...", "Assisted with...", "Tasked with...", or "Worked on...".
2. **As Measured by [Y] (The Quantifiable Proof):** Provide the specific metric that proves the achievement (percentage reduction/increase, time saved, uptime percentage, user scale, ticket volume, dollar/euro amounts, error rate, benchmark change).
3. **By Doing [Z] (The Technical Execution / Method):** Detail the exact technical action, tool, architecture, protocol, or automation you implemented to achieve the result.

### The Strict Before vs. After Transformation:

❌ **Duty/Task Bullet (REJECTED):**
"Administered 300+ user Azure/M365 tenant with access controls and IAM policies."
*(Why it fails: Lists a task. Has no accomplishment [X], has scale but no impact metric [Y], and vague action [Z].)*

✅ **Google X-Y-Z Bullet (APPROVED):**
"Strengthened identity security posture and eliminated unauthorized sign-in attempts by 100% across a 300+ user Azure/M365 tenant by engineering least-privilege RBAC, Conditional Access policies, and mandatory MFA enforcement."
*(X = Strengthened identity security posture; Y = eliminated unauthorized sign-ins by 100% across 300+ users; Z = engineered least-privilege RBAC, Conditional Access, and MFA).*

---

❌ **Duty/Task Bullet (REJECTED):**
"Automated infrastructure deployments and administrative tasks using Python and Bash."
*(Why it fails: Vague action with no proof of value or measurable business outcome.)*

✅ **Google X-Y-Z Bullet (APPROVED):**
"Reduced recurring Tier 1/2 support tickets and manual administrative workload by ~40% by authoring custom Python, Bash, and PowerShell automation scripts for user onboarding and infrastructure provisioning."
*(X = Reduced support tickets and manual administrative workload; Y = by ~40%; Z = authoring Python, Bash, and PowerShell automation scripts).*

---

❌ **Duty/Task Bullet (REJECTED):**
"Monitored and managed Wazuh SIEM for security logs."
*(Why it fails: Zero impact, reads like a job description.)*

✅ **Google X-Y-Z Bullet (APPROVED):**
"Decreased Mean Time to Detect (MTTD) security anomalies by 35% across 50+ enterprise servers by architecting a distributed Wazuh SIEM log aggregation pipeline paired with automated Grafana alert dashboards."
*(X = Decreased MTTD security anomalies; Y = by 35% across 50+ servers; Z = architecting a distributed Wazuh SIEM pipeline with Grafana dashboards).*

---

❌ **Duty/Task Bullet (REJECTED):**
"Repaired hardware and swapped motherboards on customer laptops."
*(Why it fails: Low-level task without performance indicators.)*

✅ **Google X-Y-Z Bullet (APPROVED):**
"Achieved a 94% First-Time-Fix Rate (FTFR) across 500+ on-site enterprise client hardware interventions by executing structured pre-boot diagnostics (ePSA, LED blink codes) and precision component replacements under strict ESD controls."
*(X = Achieved high First-Time-Fix Rate; Y = 94% FTFR across 500+ interventions; Z = executing structured pre-boot diagnostics and precision replacements under ESD controls).*
...
## Implementation Checklist: The Google X-Y-Z Linter

Before approving any bullet point, run it through this strict 3-step test:

1. **[X] Test (Accomplished What?):** Does the bullet open with an active achievement showing business, operational, or technical value? (If it starts with "Responsible for", "Helped", "Worked on", "Participated in", or "Supported", it FAILS).
2. **[Y] Test (How Measured?):** Does the bullet contain at least one verifiable, defensible metric (e.g. %, €, $, time reduction, volume, scale, SLA rate, error reduction)? (If there is no number or metric, it FAILS).
3. **[Z] Test (By Doing What?):** Does the bullet explain the specific technology, architecture, script, standard, or protocol utilized? (If it just says "by working hard" or "using modern tools", it FAILS).

```
FORMULA RULE:
[Action Verb + Outcome (X)] + [Quantified Metric (Y)] + [Method / Tool / System Applied (Z)]
```

### Domain-Specific Google X-Y-Z Templates:

* **System Administration / Infrastructure:**
  "Sustained [99.9% uptime / 100% service availability] across [N servers / N users] by [configuring automated failover, proactive monitoring, and patch management]."
* **Cybersecurity & SOC Operations:**
  "Reduced [MTTD / incident response times] by [X%] across [N endpoints] by [architecting SIEM correlation rules, EDR isolation workflows, and automated triage playbooks]."
* **Automation & Scripting:**
  "Eliminated [X hours / Y% manual overhead] in [process] by [developing custom Python/Bash automation scripts integrating with REST/Graph APIs]."
* **Field Service & Desktop Support:**
  "Achieved [X% First-Time-Fix Rate / resolved N monthly tickets] exceeding [SLA by Y%] by [standardizing pre-boot hardware diagnostic procedures and maintaining structured spare parts inventory]."
